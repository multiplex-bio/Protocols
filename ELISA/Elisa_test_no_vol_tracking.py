...
def run(protocol_context):
    ...
    # FIRST STEP (1/2): DISPENSING THE CAPTURE ANTIBODY AND OVERNIGHT INCUBATION
    
    # Se carga el washing reservoir con un slot con el Capture antibody 
    capture_antibody = washing_reservoir.wells()[3]
    
    
    # Se dispensa el anticuerpo en las placas que se van a usar
    m300.pick_up_tip()
    for sample in elisa_samples:
        m300.transfer(200, capture_antibody, sample.top(), new_tip='never', touch_tip=True, blow_out=True)
    m300.drop_tip()
    
    
    # Incubación overnight a 4°C
    protocol_context.comment("Retirar la placa. Incubar a 4°C Overnight")
    ...
    # FIRST STEP (2/2): WASHING THE CAPTURE ANTIBODY
    ...
    # SECONDS STEP: SAMPLE DISTRIBUTION   
    
    
    #' Modificar para que dispense líquido en una cantidad de wells = sample_number - 2
    
    
    ## LABWARE:
    # Racks with the extraction buffer
    
    # Number of racks with the extractions in eppendorf tubes and where they will be in the deck
    number_of_racks_with_extractions = math.ceil(sample_number/24) # Defining how many racks we will use
    slots_of_racks_with_extractions = ['4','5','7','8'][:number_of_racks_with_extractions] # Defining where in the deck will be those racks
    
    # List with all the racks with the extractions
    extraction_racks = [protocol_context.load_labware('opentrons_24_tuberack_nest_1.5ml_snapcap', slot, 'Extractions rack') for slot in slots_of_racks_with_extractions] 
    
    
    # List with all the eppendorfs with extraction (across all racks)
    counter_of_eppendorfs = 0
    list_of_eppendorfs_with_extractions = []
    
    for rack in extraction_racks:
        for eppendorf in rack.wells():
            if counter_of_eppendorfs < sample_number :
                counter_of_eppendorfs += 1
                list_of_eppendorfs_with_extractions.append(eppendorf)
                
    
    elisa_wells = elisa_plate.wells()[:sample_number]
    
    
    # Commands
    ## Transfering 200 uL of sample from each eppendorf of each stock plate to every well on the test_plate
    #for eppendorf, sample in zip(list_of_eppendorfs_with_extractions, elisa_wells):
    #    s300.transfer(200, eppendorf, sample.top(), touch_tip=True, blow_out=True, new_tip='Always')
    
    
    # Manual load of control samples and incubation
    #protocol_context.comment("Retira la placa y carga ctrl + y ctrl - en ella. Incubar por 2:00:00 horas a 37°C")
    
    
    
    # Washing: Removing samples with washing buffer
    #m300.pick_up_tip()
    #for key, washing_buffer in PBST_dic.items():
    #    for sample in elisa_samples:
    #        m300.transfer(200, washing_buffer, sample.top(), touch_tip=True, blow_out=True, new_tip='never')
    ...
    # THIRD STEP: DETECTION ANTIBODY
    
    #' Si son menos de 5 muestras, trabajar en eppendorf. Si es más, en reservoir.
    #' Si son 24 muestras o menos, hacerlo con la single channel
    
    # Se carga el washing_reservoir con 3 carriles con washing buffer y un slot con el detection antibody
    #detection_antibody = washing_reservoir.wells()[4]
    
    # Transfering 200 uL of Detection antibody to the ELISA test wells
    #m300.pick_up_tip()
    #for sample in elisa_samples:
    #    m300.transfer(200, detection_antibody, sample.top(), new_tip='never', touch_tip=True, blow_out=True)
    #m300.drop_tip()
    ...
    # Washing: Removing the Detection antibody with washing buffer
    #m300.pick_up_tip()
    #for key, washing_buffer in PBST_dic.items():
    #    for sample in elisa_samples:
    #        m300.transfer(200, washing_buffer, sample.top(), touch_tip=True, blow_out=True, new_tip='never')
    ...
    # FIFTH STEP: SUBSTRATE 
    #' Si son 24 muestras o menos, hacerlo con la single channel
    
    # Se carga el washing reservoir con 3 carriles con washing buffer y un slot con el Substrate
    substrate_reagent = washing_reservoir.wells()[6]
    
    # Transfering 200 uL of Substrate Reagent to the ELISA test wells
    #m300.pick_up_tip()
    #for sample in elisa_samples:
    #    m300.transfer(200, substrate_reagent, sample.top(), new_tip='never', touch_tip=True, blow_out=True)
    #m300.drop_tip()
    
    # Incubation step
    #protocol_context.comment("Retirar la placa. Cubrir con un Aluminio e incubar a temperatura ambiente por 40 minutos")
    #protocol_context.comment("Luego de eso realizar la medición de absorbancia en la placa para 405 nm")    